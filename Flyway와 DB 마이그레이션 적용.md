...
> 당신: (Git 클론 후 프로젝트를 실행한) "서버 띄웠는데 에러가 나요. `Table 'member' doesn't exist` 라고 하네요."
...
Flyway를 사용할 때 보통 컬럼 누락 같은 실수를 방지하기위해  보통 JPA 검증 옵션 과 함께 사용합니다.
> JPA 스키마 검증(hibernate.ddl-auto: validate)

이렇게 되면 만약 코드에만 컬럼추가를 반영하고 DB스키마는 건들지 않았을시 
<br>
서버 시작 시 JPA 엔티티와 실제 DB 테이블 구조를 비교하고 런타임 오류로 잡아줍니다.
<br>
그래서 이후 SQL을 직접 날리는게 아닌 그저 파일에 작성만 해두고 서버를 키면 DB의 메타데이터 테이블을 확인하고 실행되지 않은 파일을 자동으로 DB에 실행해 줍니다.