...
<br>
이 정리글은 구글, 카카오, 네이버 로그인을 구현할 때, 어떻게 객체지향적인 설계를 통해 이후 페이스북이나 인스타그램 같은 새로운 소셜 로그인이 추가되더라도 기존 코드를 수정하지 않고 유연하게 확장할 수 있는지에 대해 정리하였습니다.
...
만약 로그인 요청을 처리하는 서비스 계층에서 분기문(if-else 또는 switch)을 사용하여 각 제공자(Provider)에 맞는 로직을 처리한다면, 코드가 길어지고 복잡해집니다.
또한, 추후에 페이스북, 인스타그램 등 새로운 OAuth 제공자를 추가하거나 기존 제공자를 삭제할 때마다 핵심 비즈니스 로직(서비스 계층)의 코드를 직접 수정해야 하므로 **개방-폐쇄 원칙(OCP)**을 위배하게 됩니다.

### 해결
**전략 패턴(Strategy Pattern)**과 **팩토리 패턴(Factory Pattern)**을 도입하여 외부 API 통신 및 응답 처리를 추상화하였습니다.
> `OAuthClient`라는 공통 인터페이스를 두고 각 소셜 제공자(Google, Kakao, Naver)마다 이를 구현하도록 설계하였으며,
> `OAuthClientFactory`를 통해 요청에 맞는 구현체를 런타임에 동적으로 주입하여 사용합니다.
...
모든 OAuth 클라이언트가 공통으로 가져야 할 행위를 인터페이스로 정의합니다.
해당 인터페이스는 자신이 어떤 제공자(`Provider`)인지 식별하는 메서드와 인가 코드를 받아 유저 정보를 반환하는 메서드로 구성됩니다.

```java
public interface OAuthClient {
    // 해당 클라이언트가 어떤 소셜 로그인(구글, 카카오, 네이버 등)인지 식별
    Provider provider();
...
Spring의 의존성 주입(DI) 기능을 활용하여, `OAuthClient`를 구현한 모든 빈(Bean)을 `List`로 주입받습니다.
이후 `Provider`를 키(Key)로, 구현체(Client)를 값(Value)으로 하는 `Map`을 구성하여 
요청이 들어왔을 때 분기문 없이 O(1)의 시간 복잡도로 알맞은 클라이언트를 찾아 반환합니다.
...
public class OAuthClientFactory {
    ...
    // Spring이 OAuthClient를 구현한 모든 클래스(GoogleOAuthClient, KakaoOAuthClient 등)를 List로 주입해줌
...
## 3. 이 패턴의 장점 (새로운 OAuth 추가 시)

만약 서비스가 확장되어 **페이스북(Facebook)** 로그인을 도입해야 한다고 가정해 보겠습니다.
이러한 구조에서는 기존 컨트롤러나 서비스 계층의 로직(위 코드들)을 **단 한 줄도 수정할 필요가 없습니다.** 어떤 경로로, 어떤 제공자의 요청이 들어와도 추상화 패턴 덕분에 동일하게 처리되기 때문입니다.

단지 아래와 같이 새로운 구현체를 만들어주기만 하면 됩니다.
...
public class FacebookOAuthClient implements OAuthClient {
    ...
    @Override
    public Provider provider() {
        return Provider.FACEBOOK; // 새로운 제공자 타입 명시
    }

    @Override
    public OAuthUserInfoDto getUserInfo(String authorizationCode) {
        // 페이스북 API 연동 및 유저 정보 조회 로직 구현
        return new OAuthUserInfoDto(...); 
    }
}
```

Spring이 애플리케이션 컨텍스트를 띄울 때 `@Component` 어노테이션이 붙은 `FacebookOAuthClient`를 자동으로 빈으로 등록하고, `OAuthClientFactory`의 `List<OAuthClient>` 주입 목록에 포함시켜 줍니다.

결과적으로 다형성과 팩토리 패턴을 활용하여, **어떤 경로로 들어와도 유연하게 처리할 수 있으며, 새로운 기능(페이스북 로그인)이 추가될 때 기존 핵심 코드는 전혀 변경하지 않고 확장만 가능한 완벽한 개방-폐쇄 원칙(OCP)**을 달성할 수 있습니다.
